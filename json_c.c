...
json_value json_string_to_value(const char** json_message) {
				...
				switch(ch){
					case '\\':
					{
						...
						switch(escape){
							...
							//Parsing unicodes: \uXXXX (4 hex digits) -> 2 bytes
                            case 'u':{
                                int chs[4] = {0,};
                                int ok = 1;
                                    for(int idx=0; idx<4; idx++){
                                    char hc = tolower(*((*json_message)++));
                                    if(hc >= 'a' && hc <= 'f')      chs[idx] = hc - 'a' + 10;
                                    else if(hc >= '0' && hc <= '9') chs[idx] = hc - '0';
                                    else{
                                        fprintf(stderr, "json_string_to_value error: invalid hex in '\\u'\n");
                                        ok = 0;
                                        break;
                                    }
                                    }
    ...
    if(ok){
        str[size++] = (char)(chs[0]*16 + chs[1]);  // 상위 바이트
        str[size]   = (char)(chs[2]*16 + chs[3]);  // 하위 바이트 (아래서 size++ 됨)
    }
    break;
}
